 ...
 * @author Ivo Woltring
 */
public class Devide extends NonTerminalExpression {

    public Devide(final Expression right, final Expression left) {
        super(left, right);
    }

    @Override
    public BigDecimal interpret(final Context context) {
        return getLeft().interpret(context)
                        .divide(getRight().interpret(context));
    }
}
